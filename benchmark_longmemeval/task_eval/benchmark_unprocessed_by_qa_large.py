...
class LongMemEvalBenchmark:
    ...
    def process_single_sample(
        ...
        logger.info(f"\n{'='*80}")
        logger.info(f"[QA_{sample_idx}] {question_id}")
        ...
        try:
            # 1. 加载对话历史到记忆系统
            logger.info(f"[QA_{sample_idx}] 加载对话历史...")
            ...
            logger.info(
                ...
                f"(耗时: {timing_info['load_time']:.2f}s)"
            )
    ...
    def _save_summary(self, summary: Dict[str, Any]):
        ...
        # 保存汇总文件
        summary_file = self.output_dir / f"summary_gen_{gen_model_name}_eval_{eval_model_name}_QA{start_idx}-{end_idx}_{timestamp}.json"
    ...
    def run_benchmark_resume(
        ...
        """
        logger.info("="*80)
        logger.info("开始 LongMemEval Benchmark 测试（支持断点续传）")
        logger.info("="*80)
        logger.info(f"生成 LLM 模型: {self.gen_llm_model}")
        logger.info(f"评估 LLM 模型: {self.eval_llm_model}")
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='LongMemEval Benchmark 测试（逐条保存）')
    parser.add_argument(
        '--dataset',
        type=str,
        default='benchmark_longmemeval/dataset/LongMemEval/longmemeval_m_cleaned.json',
        help='数据集路径'
    )
    parser.add_argument(
        '--gen-model',
    ...
    parser.add_argument(
        '--output-dir',
        type=str,
        default='benchmark_longmemeval/benchmark_m_results',
        help='输出目录'
    )
    parser.add_argument(
        '--resume',
