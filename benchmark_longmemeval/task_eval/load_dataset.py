...
class LongMemEvalLoader:
    ...
    def load_sample(
        ...
        logger.info(f"[样本 {sample_idx}] 开始加载对话历史 question_id={question_id}, user_id={user_id}")
...
if __name__ == "__main__":
    ...
    # # 测试查询
    # search_result = loader.query_memory(
    #     question="How long is my daily commute to work?",
    #     sample_idx=1,
    #     user_id_base="longmemeval_s",
    #     top_k=5
    # )
    
    # print(f"\n检索到的记忆 ({len(search_result)} 条):")
    # for i, memory in enumerate(search_result, 1):
    #     print(f"\n记忆 {i}:")
    #     print(f"  ID: {memory.get('id', 'N/A')}")
    #     print(f"  Memory: {memory.get('memory', 'N/A')[:100]}...")  # 只显示前100个字符
    #     print(f"  Score: {memory.get('score', 'N/A')}")
    #     print(f"  Rerank Score: {memory.get('rerank_score', 'N/A')}")