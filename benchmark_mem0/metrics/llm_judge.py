...
client = OpenAI(
    ...
    base_url=os.getenv("OPENAI_BASE_URL")  # 这一行至关重要，否则连不上你的代理
)

# client = OpenAI()

# === 在这里直接定义 extract_json 函数 ===
def extract_json(text):
    try:
        ...
        if match:
            return match.group(1)  # <--- 修改点：直接返回字符串，不要 json.loads
        
        # 尝试匹配第一个 { ... }
        pattern = r"({[\s\S]*})"
        match = re.search(pattern, text)
        if match:
            return match.group(1)  # <--- 修改点：直接返回字符串
